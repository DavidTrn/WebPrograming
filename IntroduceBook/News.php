...
<div class="container-fluid">
	...
	<div class="row">
		<div class="col-md-12 custom-slider">
			<img src="assets/theme-sach2.jpg" alt="Sách Tiểu Thuyết" style="width:100%">	
		</div>
	</div>
...
$(document).ready(function(){
	...
	function showNews() {
		$.ajax({
			url: "GetNews.php?page=<?php if (isset($_GET['page'])) echo $_GET['page'];?>",
			type: "post",
			dataType: "json",
			async:false,
			data: {
    ...
    var space ="";
       var page="";
	    ...
	    if(index==j)
			...
			page += '<li><a href="BookCategories.php?Category=<?php if (isset($_GET['page'],$_GET['Category'])) echo $_GET['Category']."&page='+j+'"; ?>">'+j+'</a></li>' ;
    	}
    ...
    for(i = 0; i < response.length; i++) {
        ...
        out += '<div class="col-md-2 col-xs-12 col-sm-4 item-center">';
        out += '<a href="DetailNews.php?id=' + response[i].ID+'">';
        out += '<img class="my-item book" src="'+response[i].Img +'"  title="' +response[i].Title + '" alt="abc" style="width:150px; height:200px;">';
        out += '</a></div>';
        out += '<div class="col-md-9 col-xs-12 col-sm-8">';
        out += '<div class="col-md-12 col-xs-12 col-sm-8 ">';
        out += '<a href="DetailNews.php?id=' + response[i].ID+'">'+response[i].Title + '</a>';
        out += "</div>";
        out += '<div class="col-md-12 col-xs-12 col-sm-8">';
        out += response[i].Desc;
        out += "</div></div></div>";
